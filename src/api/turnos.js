...
export async function listarTurnos() {
  const res = await apiFetch(`${API_URL}/api/turnos`);
  if (!res.ok) throw new Error("Error al listar turnos");
  return res.json();
}

export async function abrirTurno() {
  const res = await apiFetch(`${API_URL}/api/turnos/abrir`, {
    method: "POST",
...
export async function obtenerActivos(turnoId) {
  const res = await apiFetch(`${API_URL}/api/turnos/${turnoId}/activos`);
  if (!res.ok) throw new Error("Error al obtener empleados activos");
  return res.json();
}

export async function asignarAuxiliar(turnoId, auxiliarId) {
  const res = await apiFetch(`${API_URL}/api/turnos/${turnoId}/auxiliar`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function retirarAuxiliar(turnoId, empleadoId) {
  const res = await apiFetch(`${API_URL}/api/turnos/${turnoId}/auxiliar/${empleadoId}`, {
    method: "DELETE",
  });
...
export async function cerrarTurno(turnoId) {
  const res = await apiFetch(`${API_URL}/api/turnos/${turnoId}/cerrar`, { method: "POST" });
